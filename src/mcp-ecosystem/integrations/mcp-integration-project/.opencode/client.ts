...
interface SuperAssistantServer {
  name: string;
  connection: {
    type: "websocket";
    port: number;
...
async function connectToMem0(port: number) {
  try {
...
function connectToSuperAssistant(port: number, name: string) {
