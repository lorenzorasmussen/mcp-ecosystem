...
class MCPClientBridge {
  ...
  async connectToServer(serverId, serverConfig) {
    ...
    try {
      ...
      serverConnection.interceptors.request.use(
        (config) => {
          logger.debug(`Making request to server: ${serverId}`, { url: config.url, method: config.method });
          return config;
        },
        (error) => {
      ...
      serverConnection.interceptors.response.use(
        (response) => {
          logger.debug(`Response from server ${serverId}:`, { status: response.status, url: response.config.url });
          return response;
        },
        (error) => {
  ...
  async fetchServerCapabilities(serverId, connection) {
    try {
      ...
      logger.warn(`Could not fetch capabilities for server ${serverId}:`, error.message);
      return null;
    }
  }
  ...
  async parseRequest(naturalLanguageRequest) {
    ...
    const matchingTools = await this.discoveryService.findToolsForQuery(naturalLanguageRequest);
    ...
    if (matchingTools.length === 0) {
      ...
      if (!targetServer) {
        throw new Error(`No suitable MCP server found for request: ${naturalLanguageRequest}`);
      }
      
      const toolCall = await this.formatToolCall(intent, naturalLanguageRequest, targetServer);
    ...
    const toolCall = await this.formatToolCall(targetTool.name, naturalLanguageRequest, {
  ...
  determineIntent(naturalLanguageRequest) {
    ...
    } else if (lowerRequest.includes('search') || lowerRequest.includes('find')) {
  ...
  async executeToolCall(serverId, toolCall) {
    ...
    const serverConnection = await this.connectToServer(serverId, await this.getServerConfig(serverId));
    ...
    try {
      ...
      const result = await this.executeWithRetry(serverConnection, toolCall, serverId);
  ...
  async executeWithRetry(connection, toolCall, serverId) {
    ...
    for (let attempt = 0; attempt <= config.mcp.maxRetries; attempt++) {
      try {
        if (attempt > 0) {
          logger.info(`Retrying tool call on server ${serverId}, attempt ${attempt}/${config.mcp.maxRetries}`);
        ...
        logger.warn(`Tool call attempt ${attempt + 1} failed on server ${serverId}:`, error.message);
      }
    }
  ...
  async processRequest(naturalLanguageRequest) {
    ...
    try {
      logger.info('Processing natural language request:', naturalLanguageRequest);
      
      // Parse the request to determine intent and target server
      const { serverId, toolCall, intent, discoveryResults } = await this.parseRequest(naturalLanguageRequest);
      ...
      logger.info(`Request processed successfully, intent: ${intent}, server: ${serverId}`);
