      ...
      try {
        ...
        for (const item of items) {
          ...
          try {
            ...
            if (stat.isDirectory() && !item.includes('node_modules') && !item.includes('.git') && !item.includes('.pnpm')) {
              walkDir(fullPath);
            ...
            console.warn(`Warning: Cannot access ${fullPath}: ${statError.message}`);
          }
        }
      } catch (dirError) {
        console.warn(`Warning: Cannot read directory ${dir}: ${dirError.message}`);
      }
    }
